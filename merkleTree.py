...
class MerkleTree:
    ...
    def getProof(self, target_value):
        target_hash = self.__returnHash(target_value)
        proof = []
        ...
        def recurse(node):
            ...
            if node.isLeaf():
                return node.data == target_hash

            if recurse(node.left):
                proof.append(('right', node.right.data))
                return True

            if recurse(node.right):
                proof.append(('left', node.left.data))
                return True

            return False

        found = recurse(self.root)
